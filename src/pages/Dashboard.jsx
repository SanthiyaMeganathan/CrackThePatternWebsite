...
export default function Dashboard() {
  const { user, sessionToken, logout, evictionMessage, setEvictionMessage } = useAuth();
  ...
  // Show eviction warning and redirect
  useEffect(() => {
    if (evictionMessage) {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        // Session eviction handled by AuthContext polling — this is a fallback
        logout();
        navigate("/login", { replace: true });
        return;
      }
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
        <div className="bg-red-950 border border-red-700 rounded-2xl p-8 max-w-md text-center">
          ...
          <p className="text-red-500 text-xs mt-4">Redirecting to login…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-blue-900 text-white px-6 py-4 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-blue-300 text-xs">Student Dashboard</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="text-xs text-blue-300 hover:text-red-400 transition border border-blue-700 hover:border-red-500 px-3 py-1.5 rounded-lg"
          >
            Logout
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            {loadingModules ? (
              ...
              <div className="p-5">
                ...
                <button onClick={fetchModules} className="mt-2 text-blue-600 text-xs hover:underline">
                  Retry
                </button>
              </div>
            ) : modules.length === 0 ? (
        ...
        <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="p-6">
                {activeModule.type === "video" && activeModule.url && (
                  ...
                  >
                    {/* Overlay to block right-click download */}
                ...
                {activeModule.type === "link" && activeModule.url && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-4">This module contains a live session link.</p>
                    <a
                ...
                {!activeModule.url && (
                  <div className="text-center py-12 text-gray-400">
                    ...
                    <p>Content for this module is not yet available. Check back soon.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Welcome card for first load */}
          {!loadingModules && modules.length === 0 && (
            <div className="bg-gradient-to-br from-blue-900 to-blue-800 rounded-2xl p-8 text-white text-center">
              <div className="text-4xl mb-3">🎉</div>
              <h3 className="text-xl font-bold mb-2">You're enrolled! Welcome aboard.</h3>
              <p className="text-blue-200 text-sm">
                Course content will be uploaded before your batch begins. You'll receive an email
                notification when new modules go live.
              </p>
            </div>
          )}
        </div>
